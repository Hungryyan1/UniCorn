...
def generate_image_with_think(
    prompt, num_timesteps=50, cfg_scale=4.0, cfg_interval=[0, 1.0], cfg_renorm_min=0., 
    timestep_shift=4.0, resolution=1024, max_length=2048, simple_think=False, 
    num_images=1, device=None
):
    ...
    # 修改：支持多图像生成
    # 复制past_key_values和相关状态以支持批量生成
    batch_past_key_values = NaiveCache(model.config.llm_config.num_hidden_layers)
    batch_newlens = newlens * num_images
    batch_new_rope = new_rope * num_images
    
    # 将单个past_key_values扩展到batch
    for layer_idx in range(model.config.llm_config.num_hidden_layers):
        if past_key_values.key_cache[layer_idx] is not None:
            ...
            # 沿batch维度复制
            batch_past_key_values.key_cache[layer_idx] = k.repeat(num_images, 1, 1)
            batch_past_key_values.value_cache[layer_idx] = v.repeat(num_images, 1, 1)
    
    generation_input = model.prepare_vae_latent(
        curr_kvlens=batch_newlens,
        curr_rope=batch_new_rope, 
...
def generate_image(
    prompt, num_timesteps=50, cfg_scale=4.0, cfg_interval=[0, 1.0], cfg_renorm_min=0., 
    timestep_shift=1.0, num_images=1, resolution=1024, device=None
):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_images", type=int, default=4, help="Total number of images to generate per prompt")
    parser.add_argument("--batch_size", type=int, default=1, help="Number of images to generate in each batch")
    ...
    # ============ 关键修改：根据world_size选择加载策略 ============
    if world_size == 1:
        # 单GPU：使用模型并行（accelerate自动分配）
        if rank == 0:
            print("Using Model Parallel strategy with accelerate")
        ...
        # 多GPU：使用数据并行（每个进程加载完整模型到自己的GPU）
        if rank == 0:
            print(f"Using Data Parallel strategy - {world_size} GPUs")
        
        # 直接创建模型（不使用init_empty_weights）
        language_model = Qwen2ForCausalLM(llm_config)
        vit_model      = SiglipVisionModel(vit_config)
        model          = Bagel(language_model, vit_model, config)
        ...
        # 加载权重
        checkpoint_path = os.path.join(args.model_path, "ema.safetensors")
        ...
        if rank == 0:
            print(f"Each rank loaded full model on its own GPU: cuda:{rank}")
    ...
    for idx in range(start, end):
        ...
        for img_idx in range(args.num_images):
            if not os.path.exists(os.path.join(sample_path, f"{img_idx:05}.png")):
                ...
                break
        if flag:
            print(f"GPU {rank} skipping generation for prompt: {prompt} (already generated)")
            continue
        ...
        # 保存think输出
        if args.think and think_output is not None:
            with open(os.path.join(outpath, "think.txt"), "w", encoding="utf-8") as f:
