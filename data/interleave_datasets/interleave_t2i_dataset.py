    ...
    def get_data_paths(self, data_dir_list, num_used_data, parquet_info):
        ...
        for data_dir, num_data_path in zip(data_dir_list, num_used_data):
            ...
            for data_path in data_paths:
                num_row_groups = 0
                if data_path in parquet_info:
                    num_row_groups = parquet_info[data_path]["num_row_groups"]
                else:
                    try:
                        fs = init_arrow_pf_fs(data_path)
                        with fs.open_input_file(data_path) as f:
                            parquet_file = pq.ParquetFile(f)
                            num_row_groups = parquet_file.num_row_groups
                    except Exception as e:
                        print(
                            f"Warning: Could not read metadata from {data_path}, skipping. Error: {e}"
                        )
                        continue
