...
class FSDPCheckpoint:
    ...
    def try_load_ckpt(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        if resume_from is not None and os.path.exists(resume_from):
            ...
            model_state_dict.pop('latent_pos_embed.pos_embed', 'latent_pos_embed.pos_embed does not exist')
            model_state_dict.pop('vit_pos_embed.pos_embed', 'vit_pos_embed.pos_embed does not exist')
            ...
            if ema_model is not None:
                ...
                # NOTE position embeds are fixed sinusoidal embeddings, so we can just pop it off,
                # which makes it easier to adapt to different resolutions.
                ema_state_dict.pop('latent_pos_embed.pos_embed', 'latent_pos_embed.pos_embed does not exist')
                ema_state_dict.pop('vit_pos_embed.pos_embed', 'vit_pos_embed.pos_embed does not exist')
