...
def process_jsonl(input_file, output_file, log_file='processing_log.txt'):
    ...
    for group_idx in range(total_groups):
        ...
        mean_score /= len(valid_items)
        
        max_items = [item for item in valid_items if item[1] == max_score]
        min_score = min(item[1] for item in valid_items)
        min_items = [item for item in valid_items if item[1] == min_score]
        selected_item, selected_score = random.choice(max_items)
        selected_item_min, selected_score_min = random.choice(min_items)
        # prompt_text = extract_prompt_text(selected_item.get('prompt', ''))
        prompt_text = selected_item.get('gen_prompt', '')
        image_path = selected_item.get('image', '')
        image_path = osp.join(image_root, image_path)
        image_path_min = selected_item_min.get('image', '')
        image_path_min = osp.join(image_root, image_path_min)
        ...
        try:
            ...
            image_bytes_min = read_image_as_bytes(image_path_min)
        except Exception as e:
            skipped_parse_error.append(
